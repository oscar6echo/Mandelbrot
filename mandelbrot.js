...
Mandelbrot.prototype = {
    ...
    form_submit: function() {
        ...
        this.escape_radius = parseFloat(document.getElementById("coord").elements["escape_radius"].value);
    ...
    key_update: function(){
        ...
        if (event.keyCode == 74 || event.keyCode == 82 || event.keyCode == 75 || event.keyCode == 70) { // not form input
            var height = this.i_max - this.i_min;
            var width = this.r_max - this.r_min;
            var shift = 1/3;
            
            if (event.keyCode == 74) {  // left -> J on keypad
                this.r_min = this.r_min - width * shift;
                this.r_max = this.r_max - width * shift;
            };
            if (event.keyCode == 82) {  // up -> R on keypad
                this.i_min = this.i_min + height * shift;
                this.i_max = this.i_max + height * shift;
            };
            if (event.keyCode == 75) {  // right -> K on keypad
                this.r_min = this.r_min + width * shift;
                this.r_max = this.r_max + width * shift;
            };
            if (event.keyCode == 70) {  // down -> F on keypad
                this.i_min = this.i_min - height * shift;
                this.i_max = this.i_max - height * shift;
            };
